export default {
  settings: {
    ...
    general: {
      ...
      globalFilter: {
        name: '全局过滤',
        desc: '应用于“最近文件”和“随机笔记”模块的过滤器。支持逻辑运算符（AND, OR, NOT）、标签（#tag）、字符串（"folder"），以及 [property:value] 形式的属性过滤，并支持 `@字段` 引用比较。',
        syntaxHelp: {
          label: '查询语法示例',
          ariaLabel: '显示全局过滤查询语法帮助',
          intro:
            '支持路径、标签、AND / OR / NOT 逻辑运算，以及 [属性] / [属性:值] 形式的过滤；属性既可来自 frontmatter，也可来自文件字段（如 path、name、basename、extension、parent.name、parent.path）；右侧还可用 `@字段` 引用其他文件字段。',
          ...
          exampleFieldReference: '示例：[basename:@parent.name]',
